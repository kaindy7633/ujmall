...
<form method="post" name="theForm" action="mail_template.php?act=save_template">
  ...
  <tr>
    ...
    <td>
      <select id="selTemplate" onchange="loadTemplate()">
        <?php echo $this->html_options(array('options'=>$this->_var['templates'],'selected'=>$this->_var['cur'])); ?>
      </select>
    </td>
  </tr>
  <tr>
    <td style="font-weight: bold; " width="15%"><?php echo $this->_var['lang']['mail_subject']; ?>:</td>
    <td><input type="text" name="subject" id="subject" style="width: 300px" value="<?php echo $this->_var['template']['template_subject']; ?>"/></td>
  </tr>
  <tr>
    <td style="font-weight: bold" ><?php echo $this->_var['lang']['mail_type']; ?>:</td>
    <td>
      <input type="radio" name="mail_type" value="0" <?php if ($this->_var['template']['is_html'] == '0'): ?>checked="true"<?php endif; ?> onclick="javascript:change_editor();"/><?php echo $this->_var['lang']['mail_plain_text']; ?>
      <input type="radio" name="mail_type" value="1" <?php if ($this->_var['template']['is_html'] == '1'): ?>checked="true"<?php endif; ?> onclick="javascript:change_editor();"/><?php echo $this->_var['lang']['mail_html']; ?>

      <input type="hidden" name="tpl" value="<?php echo $this->_var['tpl']; ?>" />
    </td>
  </tr>
  <tr>
    <td colspan="2"><?php if ($this->_var['template']['is_html'] == '1'): ?>
  ...
  <tr>
    <td colspan="2" align="center"><input type="submit" value="<?php echo $this->_var['lang']['button_submit']; ?>" class="button" /></td>
  </tr>
  </table>
</form>
