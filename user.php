...
$not_login_arr =
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer' , 'oath' , 'oath_login', 'other_login', 'phone_reg', 'email_reg');
...
/*******二次开发********************************************************************************************************/
//ajax手机注册信息处理
elseif($action == 'phone_reg'){
	 /* 检查验证码 */
	include_once('includes/cls_captcha.php');
	$validator = new captcha();
	if (!$validator->check_word($_POST['validCode'])){
		echo "validCodeError"; 
		exit();	
	}
	//获取注册信息
	if(isset($_POST['uName']) && !empty($_POST['uName'])) $uName = trim($_POST['uName']);
	if(isset($_POST['uPwd']) && !empty($_POST['uPwd'])) $uPwd = trim($_POST['uPwd']);
	//验证手机注册名
	if(!preg_match("/^13[0-9]{1}[0-9]{8}$|14[0-9]{1}[0-9]{8}$|15[0-9]{1}[0-9]{8}$|17[0-9]{1}[0-9]{8}$|18[0-9]{1}[0-9]{8}$/",$uName)){
		echo "phoneError";
		exit();
	}
	//验证密码
	...
	$reg_mothod = 0;
	//向数据库插入用户注册信息
	$sql = 'INSERT INTO ecs_users (user_name, password, mobile_phone, reg_time, reg_method) VALUES ("'.$uName.'", "'.MD5($uPwd).'", "'.$uName.'", "'.time().'", "'.$reg_mothod.'")';
    $db->query($sql);
	$userId = mysql_insert_id();
	if($user->login($uName,$uPwd)) {
		update_user_info();	
	}
	echo $userId;
}
...
elseif($action == 'email_reg'){
	...
	include_once('includes/cls_captcha.php');
	$validator = new captcha();
	if (!$validator->check_word($_POST['emailValidCode'])){
		echo "validCodeError"; 
		exit();	
	}
	//获取注册信息
	...
	$reg_mothod = 1;
	//向数据库插入用户注册信息
	$sql = 'INSERT INTO ecs_users (user_name, password, email, reg_time, reg_method) VALUES ("'.$uName.'", "'.MD5($uPwd).'", "'.$uName.'", "'.time().'", "'.$reg_mothod.'")';
    $db->query($sql);
	$userId = mysql_insert_id();
	if($user->login($uName,$uPwd)) {
		update_user_info();	
