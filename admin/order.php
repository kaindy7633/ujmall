	...
	/* wei2 add by www.68ecshop.com  end */
      ...
      /* 如果需要，发短信 */
...

        {
			...
			$content = "您的订单".$_order['order_sn']."已于".local_date("Y-m-d H:i:s",time())."发货。【68ecshop】";
...

    {
...

        {
...

            {
			  ...
			  $cost_price = floatval($_POST['cost_price'][$key]);
...

    {
       ...
       /* 如果需要，发短信 */
...

                {
					...
					$content = "订单".$order_sn."付款了。收货人:".$order['consignee'].",电话:".$order['tel']."。【68ecshop】";

                    sendSMS($GLOBALS['_CFG']['sms_shop_mobile'],$content);
				...
				if($GLOBALS['_CFG']['sms_order_payed_customer'] == '1')
...

    {
...

        {
...

            {
				...
				$sql_cost =  "UPDATE ". $GLOBALS['ecs']->table('order_goods')." as og,".$GLOBALS['ecs']->table('goods')." as g SET og.cost_price = g.cost_price " . " WHERE og.goods_id = g.goods_id"." AND og.order_id = '$order_id' and og.cost_price=0";
