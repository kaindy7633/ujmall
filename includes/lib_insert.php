...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
            ...
            $str.='<li  >
			<div class="browse_history_list"><a href="'.$goods['url'].'" target="_blank" class="pro_img"><img src="'.$goods['goods_thumb'].'" alt="'.$goods['goods_name'].'" /></a>
			<div><a  href="'.$goods['url'].'">'.$goods['short_name'].'</a><p class="salePrice34405777">'.$goods['shop_price'].'</p></div><div>
			</li>';
        }
    }
    return $str;
