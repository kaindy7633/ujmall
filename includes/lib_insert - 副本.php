...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
            ...
            $str.='<div class="browse_history_list"><a href="'.$goods['url'].'" target="_blank" class="pro_img"><img src="'.$goods['goods_thumb'].'" alt="'.$goods['goods_name'].'" /></a>
			<div><a  href="">'.$goods['short_name'].'</a><p class="salePrice34405777">'.$goods['shop_price'].'</p></div><div>';
        }
    }
    return $str;
}
    ...
    foreach ($res AS $row)
    {
        ...
        switch ($row['media_type'])
        {
            case 0: // 图片广告
                ...
                $ads[] = "<img src='$src' width='" .$row['ad_width']. "' height='$row[ad_height]'
                border='0' />";
                break;
            case 1: // Flash
                $src = (strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'], 'https://') === false) ?
