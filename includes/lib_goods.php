    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
				...
				$cat_arr[$row['cat_id']]['img']   = $row['thumb'];
	$cat_arr[$row['cat_id']]['parent_id']   = $parent_id;
...
function get_child_tree($tree_id = 0)
{
    ...
    {
        $child_sql = 'SELECT cat_id, cat_name, parent_id, is_show,thumb ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
			   ...
			   $three_arr[$row['cat_id']]['img']   = $row['thumb'];
...
$goods[$idx]['t_now']        = $time;
		...
		$goods[$idx]['zhekou']          = (number_format(intval($row['promote_price'])/intval($row['shop_price']),1))*10;
    }
 ...
 * @return  array
 */
function assign_cat_goods($cat_id, $num = 0, $from = 'web',$return='cat')    //这里增加了一个参数  $return， 下面有用到这个参数， yyy添加
{
