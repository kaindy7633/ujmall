...
*/
/************************ get_user_info()库函数原版代码区域 end *********************************************************/

/************************ get_user_info()库函数二次开发代码区域 start *********************************************************/
/**
 *  获取用户信息数组
 *
 * @access  public
 ...
 * @return array        $user       用户信息数组
 *
 * 代码修改 FOR U+易购 by LiuZhen at 2015/04/04
 */
function get_user_info($id=0)
{
    if ($id == 0)
    ...
    $time = date('Y-m-d');
    $sql  = 'SELECT u.user_id, u.email, u.user_name, u.user_money, u.pay_points, u.alias, u.user_face, u.rank_points'.
            ' FROM ' .$GLOBALS['ecs']->table('users'). ' AS u ' .
            " WHERE u.user_id = '$id'";
    $user = $GLOBALS['db']->getRow($sql);
    $bonus = get_user_bonus($id);
    ...
    $user['user_bonus']  = price_format($bonus['bonus_value'], false);
	//根据会员等级积分判断其等级
	$rank = (int)$user['rank_points'];
	if(($rank >= 0) && ($rank < 100)){
		$user['rank'] = 0;
		$user['upNeedPoints'] = 100 - $rank;
	}elseif($rank >= 100 && $rank < 10000){
		$user['rank'] = 1;
		$user['upNeedPoints'] = 10000 - $rank;
	}elseif($rank >= 10000 && $rank < 50000){
		$user['rank'] = 2;
		$user['upNeedPoints'] = 50000 - $rank;
	}elseif($rank >= 50000 && $rank < 100000){
		$user['rank'] = 3;
		$user['upNeedPoints'] = 100000 - $rank;
	}
	//统计当前用户可使用红包数量
	
    return $user;
}
/************************ get_user_info()库函数二次开发代码区域 end *********************************************************/
...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    /* 初始化“页面标题”和“当前位置” */
    $page_title = $GLOBALS['_CFG']['shop_title'] . ' - ' . 'Powered by U+';
