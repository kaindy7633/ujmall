...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
		...
		$sql="select * from ".$ecs->table('comment')." where comment_type=0 and status=1 and comment_rank!=0 and 
			id_value=$goods_id"; 
			...
			if($count2 > 0){ 
			$smarty->assign('haopinglv',  round($haoping/($count2*5)*100,1)); 
			}
			} 
			else{
				$smarty->assign('haopinglv',  "100"); 
				}
		//评价率end
		//yyy添加start
