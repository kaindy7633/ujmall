...
*/
	...
	$sql="INSERT INTO ".$ecs->table('comment')." (`comment_id`, `comment_type`, `id_value`, `email`, `user_name`, `content`, `comment_rank`, `add_time`, `ip_address`, `status`, `parent_id`, `user_id`) VALUES (NULL ,  '0',  '$goods_id',  '".$user['email']."',  '".$user['user_name']."',  '$content',  '$comment_rank',  '".time()."',  '',  '0',  '0',  '$uid');";
	$res=$db->query($sql);
	if($res){
		$result['code']=1;
