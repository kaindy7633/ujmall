...
function cart_goods($type = CART_GENERAL_GOODS)
{
    ...
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, " .
            "c.market_price, c.goods_price, c.goods_attr, c.is_real, c.extension_code, c.parent_id, c.is_gift, c.is_shipping,g.goods_thumb, " .
            "c.goods_price * c.goods_number AS subtotal " .
            "FROM " .$GLOBALS['ecs']->table('goods')."AS g,". $GLOBALS['ecs']->table('cart') .
            "as c WHERE c.goods_id = g.goods_id AND $sql_where ".
            "AND c.rec_type = '$type'";
...
function cart_goods_exists($id, $spec, $type = CART_GENERAL_GOODS)
{
    ...
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('cart').
            "WHERE $sql_where AND goods_id = '$id' ".
...
function get_cart_goods()
{
	...
	$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."'or session_id = '".SESS_ID ."'": "session_id = '" . SESS_ID . "' AND user_id=0 ";
	//$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."'": "session_id = '" . SESS_ID . "' AND user_id=0 ";
    /* 循环、统计 */
