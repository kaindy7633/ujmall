...
*/
	...
	/*订单最小金额*/
	$sql="SELECT value FROM ".$ecs->table('shop_config')." WHERE id='420'";
	$min_order_maney=$db ->getRow($sql);
	$result['min_order_maney']=$min_order_maney['value'];
	...
	if(!empty($address)){
		 ...
		 */
		 ...
		 g.goods_id,g.is_shipping,g.shop_price,g.goods_weight,g.goods_number,g.integral,g.give_integral,g.rank_integral,
			...
			g.promote_end_date,
			g.is_promote
		 FROM ". $GLOBALS['ecs']->table('goods') . " as g WHERE g.goods_id IN (".$goods_id.")";
		 $cart_goods=$GLOBALS['db']->getAll($sql);
		 
		$result['integral']=0;
		$result['rank_integral']=0;
