...
*/
	...
	if(isset($_GET['filter_attr'])&&(!empty($_GET['filter_attr']))){//筛选列表
		...
		if($field=="goods_number"){
			if(!empty($brand)){
				$sql="SELECT g.goods_id,g.goods_name,g.shop_price,g.goods_thumb,g.promote_price,g.is_promote,promote_end_date,promote_start_date,g.is_hot,g.is_new,g.is_best,g.click_count,SUM( og.goods_number ) AS goods_number FROM  ".$ecs->table('goods')." AS g,`ecs_order_goods` AS og WHERE is_delete = '0' AND is_on_sale = '1' AND g.brand_id='$brand' and g.goods_number >0 AND og.goods_id = g.goods_id AND (g.cat_id IN ($cat) OR g.goods_id IN ('') ) ".$ext." order by goods_number DESC, g.shop_price $order   LIMIT $page,10";
			...
			if(!empty($brand)){
				$sql="SELECT g.goods_id,g.goods_name,g.shop_price,g.goods_thumb,g.promote_price,g.is_promote,promote_end_date,promote_start_date,g.is_hot,g.is_new,g.is_best,g.click_count FROM  ".$ecs->table('goods')." AS g WHERE is_delete = '0' AND is_on_sale = '1' AND g.brand_id='$brand' and g.goods_number >0 AND (g.cat_id IN ($cat) OR g.goods_id IN ('') )   ".$ext." order by goods_number DESC, g.shop_price $order   LIMIT $page,10";
	...
	if($field=="goods_number"){
		if(!empty($cat)){
			if(	$cat==0){
				$sql="SELECT g.goods_id,g.goods_name,g.shop_price,g.goods_thumb,g.promote_price,g.is_promote,promote_end_date,promote_start_date,g.is_hot,g.is_new,g.is_best,g.click_count,SUM( og.goods_number ) AS goods_number FROM  ".$ecs->table('goods')." AS g,`ecs_order_goods` AS og WHERE is_delete = '0' AND is_on_sale = '1' AND g.goods_number >0 AND og.goods_id = g.goods_id order by goods_number DESC, g.shop_price $order   LIMIT $page,10 ";
			}else{
				$sql="SELECT g.goods_id,g.goods_name,g.shop_price,g.goods_thumb,g.promote_price,g.is_promote,promote_end_date,promote_start_date,g.is_hot,g.is_new,g.is_best,g.click_count,SUM( og.goods_number ) AS goods_number FROM  ".$ecs->table('goods')." AS g,`ecs_order_goods` AS og WHERE is_delete = '0' AND is_on_sale = '1' AND g.cat_id IN ($cat) and g.goods_number >0 AND og.goods_id = g.goods_id order by  goods_number DESC, g.shop_price $order   LIMIT $page,10";
			}
		}
		if(!empty($brand)){
			$sql="SELECT g.goods_id,g.goods_name,g.shop_price,g.goods_thumb,g.promote_price,g.is_promote,promote_end_date,promote_start_date,g.is_hot,g.is_new,g.is_best,g.click_count,SUM( og.goods_number ) AS goods_number FROM  ".$ecs->table('goods')." AS g,`ecs_order_goods` AS og WHERE is_delete = '0' AND is_on_sale = '1' AND g.brand_id='$brand' and g.goods_number >0 AND og.goods_id = g.goods_id order by goods_number DESC, g.shop_price $order   LIMIT $page,10";
		...
		if(!empty($cat)){
			if(	$cat==0){
				$sql="SELECT g.add_time,g.goods_id,g.goods_name,g.shop_price,g.goods_thumb,g.promote_price,g.is_promote,promote_end_date,promote_start_date,g.is_hot,g.is_new,g.is_best,g.click_count FROM  ".$ecs->table('goods')." AS g WHERE is_delete = '0' AND is_on_sale = '1' AND g.goods_number >0 order by   g.$field $order  LIMIT $page,10 ";
			}else{
				$sql="SELECT g.add_time,g.goods_id,g.goods_name,g.shop_price,g.goods_thumb,g.promote_price,g.is_promote,promote_end_date,promote_start_date,g.is_hot,g.is_new,g.is_best,g.click_count FROM  ".$ecs->table('goods')." AS g WHERE is_delete = '0' AND is_on_sale = '1' AND g.cat_id IN ($cat) and g.goods_number >0 order by  g.$field $order   LIMIT $page,10";
			}
		}
		if(!empty($brand)){
			$sql="SELECT g.add_time,g.goods_id,g.goods_name,g.shop_price,g.goods_thumb,g.promote_price,g.is_promote,promote_end_date,promote_start_date,g.is_hot,g.is_new,g.is_best,g.click_count FROM  ".$ecs->table('goods')." AS g WHERE is_delete = '0' AND is_on_sale = '1' AND g.brand_id='$brand' and g.goods_number >0 order by g.$field $order   LIMIT $page,10";
